...
export class ReservationsService {
  ...
  async handleCleanupPendingReservations() {
    this.logger.debug(
      'Ejecutando limpieza de reservas pendientes expiradas...',
    );
    ...
    if (result?.affected) {
      this.logger.log(
        `Se cancelaron ${result.affected} reservas por falta de pago.`,
      );
    }
  }
  async create(createReservationDto: CreateReservationDto) {
    ...
    const occupied =
      show.reservations?.reduce((acc, res) => {
  ...
  async findAll(query: { search?: string; status?: string; showId?: string }) {
    const { search, status, showId } = query;

    // Creamos el QueryBuilder con las relaciones
    const queryBuilder = this.reservationRepository
      .createQueryBuilder('res')
      .leftJoinAndSelect('res.show', 'show')
    ...
    // 2. Filtro de Estado
    if (status) {
      queryBuilder.andWhere('res.status = :status', { status });
    }

    // 3. Filtro por Show
    if (showId) {
      queryBuilder.andWhere('show.id = :showId', { showId: Number(showId) });
    }

    // Mantenemos el orden por creación
    queryBuilder.orderBy('res.createdAt', 'DESC');
